...
router.get("/", async (req, res) => {
  try {
    const [roles] = await pool.query(`
      SELECT ID_ROL, ROL, DESCRIPCION, FECHA_CREACION 
      FROM TBL_MS_ROLES 
      ORDER BY ID_ROL
    `);
...
router.post("/api/permisos/guardar", async (req, res) => {
  try {
    ...
    // Insertar nuevos permisos (SOLO CONSULTA)
    if (permisos && permisos.length > 0) {
      const values = permisos.map(p => [
        idRol,
        p.idObjeto,
        p.consulta ? 1 : 0,  // SOLO PERMISO_CONSULTA
...
router.post("/api/crear", async (req, res) => {
  try {
    const { rol, descripcion, usuarioAccion } = req.body;
    ...
    const [result] = await pool.query(
      `INSERT INTO TBL_MS_ROLES (ROL, DESCRIPCION, USUARIO_CREACION) 
       VALUES (?, ?, ?)`,
      [rol.trim(), descripcion || '', usuarioAccion || 'SISTEMA']
    );
  ...
  try {
    ...
    await pool.query(
      `UPDATE TBL_MS_ROLES 
       ...
       WHERE ID_ROL = ?`,
      [rol.trim(), descripcion || '', usuarioAccion || 'SISTEMA', id]
    );
