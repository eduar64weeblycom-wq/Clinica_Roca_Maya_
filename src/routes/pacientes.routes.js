...
router.get("/api/:id", async (req, res) => {
  try {
    ...
    const camposJSON = ['ALERGIAS', 'ENFERMEDADES_CRONICAS', 'CIRUGIAS_PREVIAS', 'MEDICAMENTOS_ACTUALES', 'VACUNAS'];
...
router.post("/api", async (req, res) => {
  try {
    ...
    if (alergiasArray.length > 0 || enfermedadesArray.length > 0 || cirugiasArray.length > 0 || medicamentosArray.length > 0 || vacunasArray.length > 0) {
      await pool.query(
        `
...
// ============================================================
// GET /pacientes/api/excel -> Descargar Excel con excel4node
// ============================================================
