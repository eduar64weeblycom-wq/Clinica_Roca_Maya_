...
async function guardarPermisos() {
      ...
      if (permisosData.ok && permisosData.permisos) {
        permisosData.permisos.forEach(p => {
          permisosMap[p.ID_OBJETO] = {
            ...
            insercion: p.PERMISO_INSERCION === 1,
            actualizacion: p.PERMISO_ACTUALIZACION === 1,
            eliminacion: p.PERMISO_ELIMINACION === 1
          };
        });
      }
  ...
  async function guardarPermisos() {
    ...
    items.forEach(item => {
      ...
      const insercion = item.querySelector('.perm-insercion')?.checked || false;
      const actualizacion = item.querySelector('.perm-actualizacion')?.checked || false;
      const eliminacion = item.querySelector('.perm-eliminacion')?.checked || false;
  ...
  if (rolesBody) {
    rolesBody.addEventListener('click', function(e) {
      ...
      if (btnEditar) {
        ...
        console.log(' Datos de la fila:', { id, rol, descripcion });
