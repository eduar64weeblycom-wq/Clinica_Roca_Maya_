document.addEventListener('DOMContentLoaded', async () => {
  ...
  const selectorPaciente = document.getElementById('selectorPaciente'); // input hidden con el ID
...
// ============================================================
// FUNCIÓN ESCAPE HTML - Agregar al inicio del archivo
// ============================================================
function escapeHtml(s) {
  ...
  // ============================================================
  //  NUEVO: LISTA DE VALORES - Buscar pacientes en tiempo real
  // ============================================================
  if (buscadorPaciente) {
    buscadorPaciente.addEventListener('input', () => {
      ...
      // Si borran el texto, se limpia la selección actual
      if (texto === '') {
      ...
      const filtrados = listaCompletaPacientes.filter(p => {
        ...
        return nombreCompleto.includes(texto) || String(p.ID_PACIENTE).includes(texto);
      });
    ...
    buscadorPaciente.addEventListener('focus', () => {
      ...
      const base = texto === ''
        ...
        : listaCompletaPacientes.filter(p => `${p.NOMBRES} ${p.APELLIDOS}`.toLowerCase().includes(texto) || String(p.ID_PACIENTE).includes(texto));
  ...
  function seleccionarPaciente(id, nombre) {
    ...
    // Disparamos el evento 'change' del input oculto para no romper
    // el resto del código (botón Excel, etc.) que ya lo escucha.
  ...
  //  MODIFICADO: ya no llena un <select>, guarda el arreglo en
  // memoria para que la lista de valores (buscadorPaciente) filtre
  // localmente mientras el usuario escribe.
  // ============================================================
  async function cargarPacientes() {
    try {
      ...
      if (buscadorPaciente) buscadorPaciente.placeholder = '🔍 Buscar paciente por nombre o ID...';
      if (totalPacientesCount()) totalPacientesCount().textContent = `${listaCompletaPacientes.length} pacientes registrados`;
  ...
  function totalPacientesCount() {
    return document.getElementById('totalPacientesCount');
  }
    ...
    try {
        ...
        <div class="paciente-header animate__animated animate__fadeIn">
          <div class="d-flex justify-content-between align-items-center flex-wrap">
            <div>
              ...
              <!--  NUEVO: campos ampliados del paciente -->
      ...
      if (consultas.length > 0) {
        ...
        consultas.forEach(consulta => {
          ...
          `;
 // Exámenes Complementarios (usando OBSERVACIONES)
  if (consulta.OBSERVACIONES && consulta.OBSERVACIONES.trim() !== '') {
    html += `
      <tr>
        <td colspan="5" style="background:#f7f9fb; border-left: 3px solid #17a2b8;">
          <div style="display:flex; align-items:flex-start; gap:8px; padding:4px 0;">
            <i class="fas fa-flask" style="color:#17a2b8; margin-top:2px;"></i>
            <div>
              <strong style="color:#17a2b8;">Exámenes Complementarios:</strong>
              <span style="white-space:pre-line;">${escapeHtml(consulta.OBSERVACIONES)}</span>
            </div>
          </div>
        </td>
      </tr>
  ...
  if (btnExportarExcel) {
    btnExportarExcel.addEventListener('click', function(e) {
      ...
      try {
        const url = '/excel/historial/' + pacienteActualId;
